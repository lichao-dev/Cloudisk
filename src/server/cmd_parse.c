...
int cmd_parse(UserInfo *user, char *cmd, MYSQL *conn) {
    ...
    if (strcmp(tokens[0], "cd") == 0) {
        if (count > 2) {
            ...
            // 如果没有参数，则认为切换到根目录
            handle_cd_db(user, "", conn);
        } else {
            // count=2
            handle_cd_db(user, tokens[1], conn);
        }
        return 0;
    }
    ...
    if (strcmp(tokens[0], "mkdir") == 0) {
        ...
        handle_mkdir_db(user, tokens[1], conn);
        return 0;
    }
    ...
    if (strcmp(tokens[0], "rmdir") == 0) {
        ...
        handle_rmdir_db(user, tokens[1], conn);
        return 0;
    }
    ...
    if (strcmp(tokens[0], "rm") == 0) {
        ...
        handle_rm_db(user, tokens[1], conn);
        return 0;
    }
